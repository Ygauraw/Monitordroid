...
public class DeviceAdmin extends DeviceAdminReceiver {
	 ...
	 * @param password - The new password that will be used to unlock the device
	 * 
	 * Note: The password should not contain Java escape characters such as "\" or 
	 * 		 empty quotes (""), otherwise the password will not be set as expected
	 */
	public void resetPassword(Context context, String password) {
		...
		lockDevice(context);
		Log.i("New Device Password:", password);
		}
	}
	
	/**
	 * Sets whether the device's camera is disabled
	 * @param disable - Set true to disable devices camera, false to enable device's camera
	 */
	public void disableCamera(Context context, boolean disable) {
		...
		//If the camera isn't already disabled and the user wants to disable the camera (disable is true), disable the device's camera
		if (!isCameraDisabled && disable) {
		dPM.setCameraDisabled(DeviceAdmin, disable);
		Log.i("Device Camera Disabled: ", Boolean.toString(disable));
		}
		//If the camera is already disabled and the user wants to enable the camera (disable is false), enable the device's camera
		if(isCameraDisabled && !disable) {
		dPM.setCameraDisabled(DeviceAdmin, disable);
		Log.i("Device Camera Disabled: ", Boolean.toString(disable));
		}
    ...
    /**
     * The message to display when a user attempts to disable Monitordroid as a device administrator, which will allow them 
     * to uninstall the Monitordroid application from the device. 
     */
    @Override
